...
function ensureSkillsDirectory(dir) {
  ...
  if (!fs.statSync(dir).isDirectory()) {
    throw new Error(
      'Skills path is not a directory: ' + dir + '\n' +
...
function getSkillPath(dir, name) {
  ...
  if (!normalized) {
    throw new Error(
      ...
      'Run "npx ai-skillkit list" to see available skills.'
    );
  }
...
function getFromDirectory(skillsDir, name) {
  ...
  if (!fs.existsSync(filePath)) {
    const available = listFromDirectory(skillsDir);
    throw new Error(
      'Unknown skill: "' + normalizeName(name) + '"\n' +
...
function resolveSkillNames(skillsDir, input) {
  ...
  if (!names.length) {
    throw new Error(
      ...
      'Example: skillkit.compose(["frontend", "file-reading"])\n' +
      'Run "npx ai-skillkit list" to see available skills.'
    );
  }
...
function normalizeComposeOptions(input, options) {
  ...
  if (settings.maxLength) {
    ...
    if (!isFinite(numericLength) || numericLength <= 0) {
      throw new Error(
        'compose maxLength must be a positive number.\n' +
        'Example: skillkit.compose({ maxLength: 15000 })'
      );
    }
...
function composeFromDirectory(skillsDir, input, options) {
  ...
  if (settings.maxLength && output.length > settings.maxLength) {
    throw new Error(
      'Composed prompt exceeds maxLength of ' + settings.maxLength + ' characters.\n' +
      'Current length: ' + output.length + ' characters\n' +
      'Try: Increase maxLength, use fewer skills, or remove metadata.'
    );
  }
...
function validateFile(filePath) {
  if (!String(filePath || '').trim()) {
    throw new Error(
      ...
      'Example: skillkit.validateFile("./skills/custom.md")'
    );
  }
